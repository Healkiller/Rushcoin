...
unsigned int nTargetSpacing = 120; // 2min
unsigned int nTargetSpacing_v2 = 3 * 60; //3 minute
unsigned int nStakeMinAge = 60 * 60 * 3; //3h
unsigned int nStakeMaxAge = 14 * 24 * 60 * 60; // 42 days
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
            ...
            if(nBestHeight == 0)
            {
            nSubsidy = 1600000 * COIN; //premine for swap
            }
...
int64_t GetProofOfStakeReward(int nHeight, int64_t nCoinAge, int64_t nFees)
{
    int64_t nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8);
            ...
            {
            nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 30  ;  //300%
            }
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "RushCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Rushcoin - Lets RUSH";
        CTransaction txNew;
        txNew.nTime = 1511705375;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1511705375;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = !fTestNet ? 181170 : 181170;
        ...
        assert(block.hashMerkleRoot == uint256("0xfb4ba5abc4b0b92eaf2af5ad98845dbee8e1221bab736b8175cd3e24473c5514"));
